...
const Init = async () => {
    const response = await axios.get("https://danhmuchanhchinh.gso.gov.vn/", { withCredentials: true });
    const dom = new JSDOM(response.data);
    const state = dom.window.document.querySelector('#__VIEWSTATE').value;
    return {
        state,
...
const Download = async (x) => {
    ...
    const writer = fs.createWriteStream('data.xls');
